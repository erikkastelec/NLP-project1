...
def read_classla(doc):
    ...
    doc_id = "test"  # file_path.split(os.path.sep)[-1][:-4]  # = file name without ".tsv"
    # Note: `quoting=csv.QUOTE_NONE` is required as otherwise some documents can't be read
    # Note: `keep_default_na=False` is required as there's a typo in corpus ("NA"), interpreted as <missing>
    curr_annotations = []
    word_count = 0
    char_count = 0
    ner_count = 1
    # 3	Evropska	evropski	ADJ	Agpfsn	Case=Nom|Degree=Pos|Gender=Fem|Number=Sing	_	_	_	NER=B-ORG
    splitdoc = doc.split("\n")
    for i in range(len(splitdoc)):
        line = splitdoc[i]
        if not line.startswith("#"):
            try:
                _, token, lemma, _, pos_tag, other, _, _, _, ner = line.split()
            except ValueError:
                continue
            pos_tag = "mte:" + pos_tag
            ner = ner.split("=")[1]
            if ner == "O":
                ...
                if ner in ["I-ORG", "I-PER", "I-LOC"]:
                    ref_info = curr_annotations[word_count - 1][-1]
                else:
                    ref_info = '*->' + str(ner_count) + "-1"
                    ner_count += 1
            char_count = char_count + len(token)
            curr_annotations.append((str(word_count + 1), token, lemma, pos_tag, ref_info))

            word_count += 1

    metadata = {"tokens": {}}
    for i, (tok_id, token, lemma, pos_tag, ref_info) in enumerate(curr_annotations):
        # Token is part of some mention
        if ref_info != "_":
            # Token can be part of multiple mentions
            ref_annotations = ref_info.split("|")
...
def read_coref149_doc(file_path, ssj_doc):
    ...
    return Document(doc_id, final_tokens, fixed_sents, sorted_mentions_dict(final_mentions), clusters,
                    metadata=metadata)
...
def read_corpus(name):
    ...
    if name == "coref149":
        ...
        return [read_coref149_doc(os.path.join(COREF149_DIR, f"{curr_id}.tcf"), doc_to_soup[curr_id]) for curr_id in
                doc_ids]
    else:

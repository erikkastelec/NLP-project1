...
from stanza.utils.conll import CoNLL as CoNLL2
...
class Eventify:
    ...
    def __init__(self, resolve_relcl=False, resolve_appos=False, resolve_amod=False, resolve_conj=True,
                 resolve_poss=False, simple=True, cut=True, big_args=False, strip=True, ud=dep_v2.VERSION,
                 mode="normal", language="sl"):
        ...
        if self.language == "sl":
            ...
            self.nlp = classla.Pipeline('sl',
                                        processors='tokenize,pos,lemma,depparse,ner', verbose=False,
                                        use_gpu=True)
        elif self.language == "en":
            self.nlp = stanza.Pipeline('en',
                                       processors='tokenize,pos,lemma,depparse,mwt,ner', verbose=False,
                                       use_gpu=True)
    ...
    def generateConll(self, text):
        ...
        if self.language == "sl":
            ...
            conll_list = CoNLL2.convert_dict(dicts)
            for sentence in conll_list:
                sentence_conll = ''

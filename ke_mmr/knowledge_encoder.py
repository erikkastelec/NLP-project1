...
class KnowledgeEncoder:
    ...
    def encode_knowledge(self, sentence, event1_offset, event2_offset):
        ...
        return temp2, (e1_start, e1_end), (e2_start, e2_end)



    # does not work
    def encode_knowledge_markers(self, sentence, event1_offset, event2_offset):
        tokens = self.tokenizer.tokenize(sentence)
        tokens = ['[CLS]'] + tokens + ['[SEP]']
        event1_offset = (event1_offset[0] + 1, event1_offset[1] + 1)
        event2_offset = (event2_offset[0] + 1, event2_offset[1] + 1)
        ...
        xx = self.tokenizer.convert_tokens_to_ids(['[E1]'] + self.tokenizer.tokenize(event1_knowledge) + ['[/E1]'])
        xx_len = len(xx)
        yy = self.tokenizer.convert_tokens_to_ids(['[E2]'] + self.tokenizer.tokenize(event2_knowledge) + ['[/E2]'])
        yy_len = len(yy)
